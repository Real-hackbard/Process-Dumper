...
type
  ...
  private
    ...
    procedure Initialize;
    procedure Update(ACol, ARow: Integer);
...
implementation
uses Consts,ComStrs,UMain, UObjectTree, RTLConsts;
type
  PIntArray = ^TIntArray;
  ...
  with FGrid.FInspector do begin
     ...
     case tmpG.EditType of
          ...
          edPointer:begin
             ...
             if PTI=TypeInfo(TIcon)then SetPropDialog(PTI);
          ...
          edPointerChange:begin
             ...
             if PTI.Name='Hint' then SetPropDialog(PTI);
             if PTI.Name='EditMask' then SetPropDialog(PTI);
          end;
     end;
  end;
end;
...
function TtsvSparseList.ForAll(ApplyFunction: Pointer ): Integer; assembler;
asm
  ...
  begin
    ...
    if PPI<>nil then begin
     if PPI^.PropType^.Kind=tkClass then begin
      ...
      if not (tmpObj is TStrings) then
...
begin
  MessageBeep(0);
  MessageBox(Application.Handle,'Неверное значение свойства.','Ошибка',MB_ICONERROR+MB_OK);
end;
...
begin
   ...
   if PTInfo=TypeInfo(TStrings)then begin
     ...
     try
      ...
      if Obj is TStrings then
       SLE.Memo.Lines.Assign(TStringList(Obj));
      if SLE.ShowModal=mrOk then begin
        TStringList(Obj).Assign(SLE.Memo.Lines);
      end;
     finally
      SLE.Free;
     end;
   ...
   if PTInfo=TypeInfo(TIcon)then begin
     PE:=TPictDialog.Create(Owner);
     try
      PTI:=FValueObj.ClassInfo;
      PPI:=GetPropInfo(PTI,PropName);
      obj:=TObject(getOrdprop(FValueObj,PPI));
      if Obj is TIcon then PE.Image.Picture.Icon.Assign(TIcon(Obj));
      if PE.Execute then begin
        TIcon(obj).Assign(PE.Image.Picture.Icon);
      end;
     finally
      PE.Free;
     end;
...
begin
  ...
  if fmObjectTree<>nil then begin
   if FValueObj is TComponent then
    fmObjectTree.ChangeTreeNode(TComponent(FValueObj));
  end;
  if FGridE<>nil then begin
