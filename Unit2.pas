...
type
  ...
  private
    ...
    procedure OperatinsFromLV(Index,IndexR: Integer;Value: string);
  ...
  public
    ...
    procedure FillGrid(proc_id:DWord; lpAddress: Integer; SizeAdrr: integer; ROnly: Boolean);
...
const
  ...
  SaveCaption='Saving...';
implementation
uses UPInfoSInfo, USearch, UProgress;
{$R *.DFM}
...
procedure TfmDumpMem.FillGrid(proc_id:DWord; lpAddress: Integer; SizeAdrr: integer; ROnly: Boolean);
var
  ProcId: LongInt;
  PH: THandle;
  MemSize: LongInt;
...
begin
  ...
  if PH=0 then begin
   MessageBox(Handle,Pchar('Process closed'),Pchar('Warning'),MB_ICONWARNING);
   exit;
  end;
  MemSize:=SizeAdrr;
...
begin
  ...
  VirtualQueryEx(
   ...
   lbAllocationProtect.Caption:=getAllocationProtect(P.AllocationProtect);
   ...
   CloseHandle(PH);
   case P.Protect of
     PAGE_READWRITE: FillGrid(procid,CurrMemAddr,PageSize,false);
    else begin
      FillGrid(procid,CurrMemAddr,PageSize,true);
    end;
   end;
end;
   ...
   begin
    ...
    if (ACol=0) or (ARow=0) then begin
     if not((ACol=0) and (ARow=0)) then
      TextOut(Rect.Left + Offset, Rect.Top+dy, sg.Cells[ACol,ARow])  
    end; 
   end;
  end;
begin
...
begin
  ...
  if Li<>nil then begin
    but:=MessageBox(fmProgress.Handle,Pchar('Address '+tmps+' already exists.'+#13+'Rewrite ?'),
...
begin
  try
    ...
    MessageBox(handle,Pchar('Page no access !'),nil,MB_ICONERROR);
  end;
end;
...
begin
  if Question then begin
   if MessageBox(Handle,Pchar('Apply changes ?'),
                        Pchar('Confirm'),
...
begin
  ...
  if ProcHand = 0 then
    raise Exception.Create('No information available for this process');
  try
...
begin
  ...
  wr.WriteString(ProcessEntry.szExeFile);
...
begin
  ...
  try
   ExeN:=rd.ReadString;
   if UpperCaseEX(ExeN)<>UpperCaseEX(ProcessEntry.szExeFile) then begin
     if MessageBox(Handle,Pchar('Loading list for Process: '+#13+ExeN),
   ...
   MessageBox(handle,Pchar('Bad format file: '+ExtractFilename(od.FileName)),nil,MB_ICONERROR);
...
begin
  ...
  if fm.ShowModal=mrOk then begin
    ...
    OperatinsFromLV(Index,IndexR,fm.edValue.Text);
    SetColumnWidth;
  end;
  fm.Free;
end;
procedure TfmDumpMem.OperatinsFromLV(Index,IndexR: Integer;Value: string);
var
  i: Integer;
  li: TlistItem;
begin
  for i:=LV.Items.Count-1 downto 0 do begin
    ...
    case Index of
      0: begin
       if Li.SubItems.Strings[2]=Value then
         case IndexR of
           0: Li.Delete;
           1: Li.Selected:=true;
           2: Li.Checked:=true;
           3: Li.SubItems.Strings[2]:=Value;
         end;
      end;
      1: begin
       if Li.SubItems.Strings[1]=Value then
         case IndexR of
           0: Li.Delete;
           1: Li.Selected:=true;
           2: Li.Checked:=true;
           3: Li.SubItems.Strings[1]:=Value;
         end;
      end;
      2: begin
       if Li.SubItems.Strings[0]=Value then
         case IndexR of
           0: Li.Delete;
           1: Li.Selected:=true;
           2: Li.Checked:=true;
           3: Li.SubItems.Strings[0]:=Value;
         end;
      end;
      3: begin
       if Li.Caption=Value then
         case IndexR of
           0: Li.Delete;
           1: Li.Selected:=true;
           2: Li.Checked:=true;
      ...
      4: begin
       if Li.SubItems.Strings[3]=Value then
         case IndexR of
           0: Li.Delete;
           1: Li.Selected:=true;
           2: Li.Checked:=true;
           3: Li.SubItems.Strings[3]:=Value;
         end;
      end;
    end;
  end;
...
begin
  ...
  if ShowTime then begin
    MessageBox(Self.Handle,Pchar('Time is: '+TimeTostr(t2-t1)),Pchar('Info'),MB_ICONINFORMATION);
  end;
end;
...
begin
  ...
  if DiskFree(0)<(ToAd-FromAd) then begin
    MessageBox(Handle,Pchar('Does not suffice disk space.'),Pchar('Error'),Mb_Ok+MB_Iconerror);
    exit;
  end;
  self.Enabled:=false;
  breaksearch:=false;
